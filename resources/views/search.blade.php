...
<div class="container-xl w-100 p-3">
	<form method="post" action="{{route('search')}}">
		...
		<div class="input-group no-border">
			<input type="text" value="" name="item" class="form-control" onmouseover="this.focus();" placeholder="Search...">
			...
			<table class="table table-striped table-hover">
				...
				<tbody>
					...
					<tr>
						...
						<td>{{$use->model}}</td>
						<td>{{$use->quantity}}</td>
						<td><span>&#8358;</span> {{$use->price}}</td>
						<td><span>&#8358;</span> {{$use->cost_price}}</td>
						{{-- <td>{{$use->store->getName() ?? 'default'}}</td> --}}
						<td>
							@isset($cart)
							...
							<form method="post" action="{{route('cart.add')}}">
								...
								<button class="btn btn-success" data-toggle="modal"><i class="material-icons" data-toggle="tooltip" title="Add to cart">cart </i></button>
							</form>
							@endif
							@endisset
